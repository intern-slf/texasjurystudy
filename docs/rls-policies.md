...
3. Possible role values seen in code: `admin`, `participant`, `requestee`, `blacklisted`.
...
Most admin/requestee/participant policies will need to check the user's role. Define this once and reuse:
...
- Requestee: read sessions they're attached to via `client/lib/actions/requesteeParticipant.ts:163,189`

**Recommended policy:**
- Enable RLS.
- `SELECT`: admin (all rows), or any authenticated user whose `user_id` appears in `session_participants` for that `session_id`, or requestees attached via `session_cases`.
...
| requestee can read own sessions | SELECT | authenticated | `EXISTS (session_cases sc JOIN cases c WHERE sc.session_id = sessions.id AND (c.user_id = auth.uid() OR c.requestee_id = auth.uid()))` | — |

Matches recommendation. Admin go-through-anon works via `is_admin()`. Requestee check correctly handles both `c.user_id` and `c.requestee_id` (the only table that does — see F16).
...
- Read-only for participants/requestees via various dashboards (`client/app/dashboard/requestee/[caseId]/page.tsx:61`, `client/components/RequesteeParticipantHistory.tsx:95`, `client/components/CaseParticipantSummary.tsx:33`)
...
| requestee can read session_cases of own cases | SELECT | authenticated | `EXISTS (cases c WHERE c.id = session_cases.case_id AND (c.user_id = auth.uid() OR c.requestee_id = auth.uid()))` | — |

Matches recommendation. No general-authenticated SELECT — narrower than the "any auth user can read" option from the recommendation, which is fine (requestee scope covers the actual reads).
...
- Requestee: `client/lib/actions/requesteeParticipant.ts:49,196` (reads participants assigned to their cases)
...
- `SELECT`:
  ...
  - Requestee: only rows where the row's `session_id` is joined via `session_cases` to a case whose `requestee_user_id = auth.uid()`. *This may require a `security definer` view or function.*
- `UPDATE`:
  - Admin: all columns.
...
**Code access:** read-only from many places (`client/lib/case-lineage.ts`, `client/components/{CaseParticipantSummary,RequesteeParticipantHistory,PreviousParticipantsModal,SelectAllParticipants}.tsx`, `client/app/dashboard/Admin/sessions/page.tsx:255`). Writes only in `client/lib/actions/session.ts:499,1016` and `client/lib/participant/updateInviteStatus.ts:261`.

**Confirmed live (2026-05-16):** Despite the name, `oldData` is actively used as a fallback store for legacy participants. The app falls back to it whenever a participant lookup misses in `jury_participants`. Used in `client/lib/case-lineage.ts`, `client/lib/actions/session.ts`, `client/lib/participant/getParticipantProfile.ts`, `client/components/{CaseParticipantSummary,RequesteeParticipantHistory,PreviousParticipantsModal,SelectAllParticipants}.tsx`, and admin/participant pages via `?test_table=oldData` URL params. **Not a deletion candidate.** Needs RLS turned on.

**Recommended policy:**
- Enable RLS.
- `SELECT`: admin + requestee (used in requestee participant-history view). If the table contains PII, restrict requestee SELECT to participants attached to that requestee's cases via the same join used for `jury_participants`.
- `INSERT` / `UPDATE` / `DELETE`: admin only.
...
| requestee can read oldData | SELECT | authenticated | `EXISTS (roles WHERE roles.user_id = auth.uid() AND roles.role = 'requestee')` | — |
...
**Code access:** admin pages list and edit, requestees read their own approved cases. Inserts via `client/lib/actions/adminCase.ts` (need to confirm). Read in `client/app/dashboard/Admin/layout.tsx:42`.

**Recommended policy:**
- `SELECT`: admin (all), requestee (only `requestee_user_id = auth.uid()`).
- `INSERT` / `UPDATE` / `DELETE`: admin only.
...
| requestee can create case | INSERT | authenticated | — | `auth.uid() = user_id` |
...
| requestee can read own cases | SELECT | authenticated | `auth.uid() = user_id` | — |
...
- Requestee SELECT keys on `user_id` only, not `requestee_id` — F16 (requestee assigned by admin to a case they didn't create can't see it).

---

### 🟩 `case_documents` — RLS enabled

**Code access:** uploaded files attached to cases. Read by requestees and admins.

**Recommended policy:** same shape as `cases` — readable by admin + the case's requestee; writable by admin (or whoever owns the document upload — confirm flow).
...
- Owner-based scoping (`uploaded_by`) instead of case-based — fine in practice as long as requestees only upload to their own cases.
...
| Policy | Cmd | Role | USING | WITH CHECK |
|---|---|---|---|---|
| Requestee manages own drive links | ALL | authenticated | `uploaded_by = auth.uid()` | `uploaded_by = auth.uid()` |
| Authenticated users can view drive links | SELECT | authenticated | `true` | — |

**Diff from recommendation:** 🟥 **NEW FINDING — see F21.** The "Authenticated users can view drive links" SELECT policy has `USING (true)` — any authenticated user can read every case's Google Drive URLs. Requestees' confidential case material exposed to participants. Drop this policy; the per-owner `ALL` policy already covers the legit read path. Admins/other requestees reading via cases dashboard should go through a case-scoped SELECT instead.
...
Confirmed full name. Requestee-side version of `confidentiality_agreements`.

**Code access:**
- Read: `client/app/dashboard/page.tsx:73,74,124` (requestee dashboard checks if requestee has signed) and `client/app/dashboard/Admin/[caseId]/page.tsx:128` (admin reads via `supabaseAdmin`).
- Write: not found in codebase yet — requestee signing flow may write here, double-check the relevant form component.

**Recommended policy:**
- `SELECT`: admin (all), requestee (only their own row).
- `INSERT`: requestee, with `WITH CHECK (user_id = auth.uid())`.
...
| Policy | Cmd | Role | USING | WITH CHECK |
|---|---|---|---|---|
| requestee can insert own agreement | INSERT | public | — | `auth.uid() = user_id` |
| requestee can read own agreement | SELECT | public | `auth.uid() = user_id` | — |
...
| Requestees can view participants in their sessions | SELECT | public | `auth.uid() = user_id OR is_admin OR EXISTS (session_participants sp JOIN session_cases sc JOIN cases c WHERE sp.participant_id = jury_participants.user_id AND (c.user_id = auth.uid() OR c.requestee_id = auth.uid()))` | — |
...
### ⬛ `requestee_responses` — DROPPED (2026-05-16)

Confirmed unused (0 code references, 0 rows, no FKs/functions/views depending on it). Dropped via `DROP TABLE public.requestee_responses` in Supabase SQL Editor on 2026-05-16. Verified absent from `information_schema.tables` post-drop.
...
|---|---|---|---|---|
| F1 | ✅ CLOSED | `sessions` RLS enabled 2026-05-17. Dropped three duplicate over-broad INSERT policies (all `WITH CHECK true`). Added admin FOR ALL via `is_admin()` + requestee SELECT scoped to sessions involving their own cases (join via `session_cases` → `cases`) + participant SELECT scoped to sessions they're invited to (via `session_participants`). Cron unaffected (uses supabaseAdmin). | | Closed |
| F2 | ✅ CLOSED | `session_cases` RLS enabled 2026-05-17. Dropped over-broad "allow authenticated insert" (WITH CHECK true). Added admin FOR ALL via `is_admin()` + requestee SELECT scoped to own cases (`c.user_id = auth.uid() OR c.requestee_id = auth.uid()`). No participant policy needed — invite flow uses supabaseAdmin. | | Closed |
| F3 | ✅ CLOSED | `session_participants` RLS enabled 2026-05-17. Dropped broken "admin can insert session invites" (referenced empty `profiles.role`) + duplicate participant UPDATE policy. Added admin FOR ALL via `is_admin()`. Kept existing participant read/update own + requestee read for own cases. | | Closed |
...
1. **Patch the 4 unrestricted tables first.** For each:
   ...
   - Smoke-test the corresponding flow end-to-end **before deploying** (admin creates a session, participant accepts an invite, requestee views their case, cron runs).
...
3. **Drop unused tables** — run the pre-flight check + `DROP TABLE` for `requestee_responses` and `transcript_orders` (decision logged 2026-05-16).
...
5. Re-test the four flows after every policy change. The biggest risk in this work is breaking admin/participant/requestee flows, not the policies themselves.
