...
- RESOLVED 2026-06-09 (F23 in [[docs-rls-policies]], new Storage section): the fear was real. All buckets were private + RLS on, but 7 of 8 `storage.objects` policies were `bucket_id`-only with no owner check → any authenticated participant could read every driver-license image (`id-documents`) and every case doc (`case-documents`). Fixed by dropping the broad policies, keeping the `owner`-scoped ALL policy, and adding admin-only SELECT. Storage now audited.
